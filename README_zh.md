...
- **多模型团队。** Claude、GPT、Gemini、Grok、MiniMax、DeepSeek、LiteLLM、Copilot，或任何 OpenAI 兼容的本地模型（Ollama、vLLM、LM Studio、llama.cpp）可以在同一个团队中使用。让架构师用 Opus 4.6，开发者用 GPT-5.4，评审用本地的 Gemma 4，一次 `runTeam()` 调用全部搞定。Gemini 作为 optional peer dependency 提供：使用前需 `npm install @google/genai`。
...
- `LITELLM_API_KEY`（LiteLLM — 本地代理可选）
- `LITELLM_BASE_URL`（LiteLLM — 可选，默认：http://localhost:4000）
...
- [19 — LiteLLM](examples/19-litellm.ts)：通过本地 LiteLLM 代理的三智能体团队。
...
| LiteLLM 代理 | `provider: 'litellm'` | `LITELLM_API_KEY`（可选） | 已验证 |
...
任何 OpenAI 兼容 API 均可通过 `provider: 'openai'` + `baseURL` 接入（Groq、Mistral、Qwen 等）。**Grok、MiniMax、DeepSeek 和 LiteLLM 现已原生支持**，分别使用 `provider: 'grok'`、`provider: 'minimax'`、`provider: 'deepseek'` 和 `provider: 'litellm'`。
...
const litellmAgent: AgentConfig = {
  name: 'litellm-agent',
  provider: 'litellm',
  model: 'gpt-4o',  // 需匹配代理配置中的别名
  systemPrompt: '你是一个有用的助手。',
}
```

使用 `litellm --model openai/gpt-4o` 启动本地代理（或任何支持的后端）。设置 `LITELLM_BASE_URL` 以连接远程部署；如为认证代理，设置 `LITELLM_API_KEY`。
